...
static int bcm2835_pll_on(struct clk_hw *hw)
{
	...
	pr_info("xzl: %s: %s", __func__, data->name);
//	if (!strcmp(data->name, "plla")) {
//		pr_err("xzl: %s: skip plla", __func__);
//		return 0;
//	}
...
static int bcm2835_clock_on(struct clk_hw *hw)
{
	...
	pr_debug("xzl:%s %s", __func__, data->name);
					   ...
					   const void *data)
{
	...
	pr_info("xzl: %s: %s", __func__, pll_data->name);
					     ...
					     const void *data)
{
	...
	pr_info("xzl: %s: %s", __func__, cprman->dev->init_name);
