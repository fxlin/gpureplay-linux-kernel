				      ...
				      u32 tag, u32 *val)
{
	...
	pr_debug("xzl: %s %s tag %08x val %08x",
			__func__, rpi_firmware_clk_names[data->id], tag, *val);
	...
	*val = le32_to_cpu(msg.val);
	...
	if (tag == 0x00030001)
		pr_debug("xzl: AFTER %s %s tag %08x val %08x",
					__func__, rpi_firmware_clk_names[data->id], tag, *val);
					       ...
					       unsigned int id)
{
	...
	pr_debug("xzl: %s %s", __func__, rpi_firmware_clk_names[id]);
