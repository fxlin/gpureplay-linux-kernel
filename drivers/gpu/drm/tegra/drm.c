		     ...
		     struct drm_file *file)
{
	...
	err = host1x_job_submit(job); // xzl: submit via host1x
	if (err) {
...
void *tegra_drm_alloc(struct tegra_drm *tegra, size_t size, dma_addr_t *dma)
{
	...
	if (!tegra->domain) { // xzl: seems iommu is optional
		/*
...
// xzl: seems to suggest iommu is optional. and will only be used
// when both gpu & host1x are behind iommus
