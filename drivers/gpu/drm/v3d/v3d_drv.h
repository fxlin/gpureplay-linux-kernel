...
#define NUM_BO_SLOTS 512 // xzl: alexnet needs >64; resnet18>256
...
struct v3d_dev {
	...
	void __iomem *core_regs[3]; // xzl: max 3 cores??
	...
	struct {
		u32 num_allocated;
		u32 pages_allocated;
		struct v3d_bo * bos[NUM_BO_SLOTS]; // xzl: for recording. track allocated bos
	} bo_stats;
};
...
struct v3d_bo {
	...
	u32 type; // xzl, from ioctl create bo
...
#define V3D_READ_NOREPLAY(offset) ({							\
	uint32_t val = readl(v3d->hub_regs + offset); 	\
	trace_v3d_access_reg(offset, val, 'r', "~hub", -1);				\
	val; 											\
})

// no tracepoint at all
#define V3D_READ_NOTRACE(offset) ({							\
	uint32_t val = readl(v3d->hub_regs + offset); 	\
	val; 											\
})
...
#define V3D_WRITE(offset, val) 							\
	do {												\
		trace_v3d_write_reg(offset, val);				\
		writel(val, v3d->hub_regs + offset); 			\
	} while (0)
#endif

#define V3D_WRITE(offset, val) 							\
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "hub", -1);	\
		writel(val, v3d->hub_regs + offset); 			\
	} while (0)

#define V3D_WRITE_NOREPLAY(offset, val) 							\
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "~hub", -1);	\
		writel(val, v3d->hub_regs + offset); 			\
	} while (0)

#define V3D_WRITE_NOTRACE(offset, val) 							\
	do {												\
		writel(val, v3d->hub_regs + offset); 			\
	} while (0)
...
#define V3D_BRIDGE_READ(offset) ({							\
	uint32_t val = readl(v3d->bridge_regs + offset); 	\
	trace_v3d_access_reg(offset, val, 'r', "bridge", -1);				\
	val; 											\
})
// #define V3D_BRIDGE_WRITE(offset, val) writel(val, v3d->bridge_regs + offset)
#define V3D_BRIDGE_WRITE(offset, val) \
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "bridge", -1);	\
		writel(val, v3d->bridge_regs + offset); 			\
	} while (0)
...
#define V3D_GCA_READ(offset) ({							\
	uint32_t val = readl(v3d->gca_regs + offset); 	\
	trace_v3d_access_reg(offset, val, 'r', "gca", -1);				\
	val; 											\
})
// #define V3D_GCA_WRITE(offset, val) writel(val, v3d->gca_regs + offset)
#define V3D_GCA_WRITE(offset, val) \
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "gca", -1);	\
		writel(val, v3d->gca_regs + offset); 			\
	} while (0)

// --------------------------------------------------------------- //
// #define V3D_CORE_READ(core, offset) readl(v3d->core_regs[core] + offset)
#define V3D_CORE_READ(core, offset) ({							\
	uint32_t val = readl(v3d->core_regs[core] + offset); 	\
	trace_v3d_access_reg(offset, val, 'r', "core", core);				\
	val; 											\
})

#define V3D_CORE_READ_NOREPLAY(core, offset) ({							\
	uint32_t val = readl(v3d->core_regs[core] + offset); 	\
	trace_v3d_access_reg(offset, val, 'r', "~core", core);				\
	val; 											\
})

#define V3D_CORE_READ_NOTRACE(core, offset) ({							\
	uint32_t val = readl(v3d->core_regs[core] + offset); 	\
	val; 											\
})

/* ok to read diff val */
#define V3D_CORE_READ_RELAXED(core, offset) ({							\
	uint32_t val = readl(v3d->core_regs[core] + offset); 	\
	trace_v3d_access_reg(offset, val, 'R', "core", core);				\
	val; 											\
})
...
#define V3D_CORE_WRITE(core, offset, val) \
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "core", core);	\
		writel(val, v3d->core_regs[core] + offset); 			\
	} while (0)

#define V3D_CORE_WRITE_NOREPLAY(core, offset, val) \
	do {												\
		trace_v3d_access_reg(offset, val, 'w', "~core", core);	\
		writel(val, v3d->core_regs[core] + offset); 			\
	} while (0)

#define V3D_CORE_WRITE_NOTRACE(core, offset, val) \
	do {												\
		writel(val, v3d->core_regs[core] + offset); 			\
	} while (0)
...
struct v3d_bin_job {
	...
	u32 start, end; // xzl: @end seems inclusive
	...
	u32 qma, qms, qts;  // xzl: qts seems to point to a TSDA
};
...
void v3d_irq_fini(struct v3d_dev* v3d);
