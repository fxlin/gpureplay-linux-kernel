...
v3d_clean_caches(struct v3d_dev *v3d)
{
	...
	// xzl
	trace_v3d_wait_for_reg(V3D_CTL_L2TCACTL, "core",
			V3D_L2TCACTL_L2TFLS, 0x0, core);

	if (wait_for(!(V3D_CORE_READ_NOREPLAY(core, V3D_CTL_L2TCACTL) &
		    ...
		    struct drm_file *file_priv)
{
	...
	// xzl: if binner cl non empty, cr a binner job...
	...
	if (bin) {	// xzl: push the binner job... add a fence to be waited? 
		     ...
		     struct drm_file *file_priv)
{
	...
	// xzl: cfg[5]: code addr, cfg[6]: uniforms addr? 
	// cf: V3D_CSD_QUEUED_CFG5
