...
static int bcm2835_pm_probe(struct platform_device *pdev)
{
	...
	pm->base = devm_ioremap_resource(dev, res);		// xzl: mapped here
	if (IS_ERR(pm->base))
		return PTR_ERR(pm->base);

	pr_info("xzl: %s map base  %x to %llx", __func__,
							(u32)res->start, (u64)pm->base);
	...
	pr_info("xzl: %s map rpivid_asb  %x to %llx", __func__,
					(u32)res->start, (u64)pm->rpivid_asb);
	...
	pr_info("xzl: %s map asb %x to %llx", __func__,
						(u32)res->start, (u64)pm->asb);
