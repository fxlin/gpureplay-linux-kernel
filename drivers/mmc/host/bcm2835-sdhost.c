...
#define SDVDD  0x30 /* SD card power control           -  1 R/W */ // xzl: on/off
#define SDEDM  0x34 /* Emergency Debug Mode            - 13 R/W */ // xzl: probing internal fsm?
#define SDHCFG 0x38 /* Host configuration              -  2 R/W */
#define SDHBCT 0x3c /* Host byte count (debug)         - 32 R/W */ // xzl: not needed by us?
#define SDDATA 0x40 /* Data to/from SD card            - 32 R/W */
#define SDHBLC 0x50 /* Host block count (SDIO/SDHC)    -  9 R/W */ // xzl: not needed by us?

#define SDCMD_NEW_FLAG                  0x8000 // xzl: a cmd is pending?
...
// xzl: state machine
#define SDEDM_FSM_MASK           0xf // xzl: bits for fsm... 16 states?
...
static void bcm2835_sdhost_reset_internal(struct bcm2835_host *host)
{
	...
	bcm2835_sdhost_set_power(host, false); // xzl: power off. then config
	...
	/* Limit fifo usage due to silicon bug */ // xzl: how does this work? why delay?
	...
	mdelay(10);
	bcm2835_sdhost_set_power(host, true); // xzl: power back on. then set cfg.
	mdelay(10);
	host->clock = 0;
	host->sectors = 0;
...
// xzl: probing internal fsm (SDEDM) until completion??
...
static void bcm2835_sdhost_read_block_pio(struct bcm2835_host *host)
{
	...
	while (blksize) {
		...
		while (copy_words) {
			...
			edm = bcm2835_sdhost_read(host, SDEDM);
			words = ((edm >> 4) & 0x1f); //xzl:w/r threshold?? cf: SDEDM_...
...
// xzl: use iteration to copy data
					  ...
					  unsigned long *irq_flags)
{
	...
	if (!retries) { // xzl: calibration. measure the # of retries in 1 us?
		...
		do {
			...
			for (i = 0; i < retries; i++) {
				cpu_relax();
				sdcmd = bcm2835_sdhost_read(host, SDCMD); // xzl: for calibration
			}
...
// xzl: register itself to the mmc framework
int bcm2835_sdhost_add_host(struct bcm2835_host *host)
{
	...
	INIT_WORK(&host->cmd_wait_wq, bcm2835_sdhost_cmd_wait_work); // xzl: entry func?
...
static int bcm2835_sdhost_probe(struct platform_device *pdev)
{
	...
	mmc->ops = &bcm2835_sdhost_ops;	// xzl: here, register callbacks
	host = mmc_priv(mmc);
	host->mmc = mmc;
