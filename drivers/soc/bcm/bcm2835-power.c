...
// xzl: power "base" regs, for a variety of power domains
...
#define PM_IMAGE			0x108		// xzl: seems a power domain
// xzl: it seems a power domain.
// before v3d enable: val= 0000_1000; after 0000_1040 (v3drstn=1)
#define PM_GRAFX			0x10c
#define PM_PROC				0x110
#define PM_ENAB				BIT(12)	// xzl: what for?
// xzl: bits 8-6 (rstn, reset neg?) seems common for IMG/GRAFX
...
#define ASB_REQ_STOP                    BIT(0) //write 0 --> start the hw?
#define ASB_ACK                         BIT(1)	//read 0 --> hw ack'd?
...
struct bcm2835_power {
	struct device		*dev;
	/* PM registers. */
	void __iomem		*base;	// xzl: when mapped?
	/* AXI Async bridge registers. */
	void __iomem		*asb;		// xzl: when mapped?

	bool is_2711;		// xzl: support bcm2711?
...
static int bcm2835_power_power_on(struct bcm2835_power_domain *pd, u32 pm_reg)
{
	...
	dev_info(power->dev, "xzl: %s 1: %s\n", __func__, pd->base.name);

	/* 2711 has no power domains above the reset controller. */
	if (power->is_2711)
		return 0;

	dev_info(power->dev, "xzl: %s 2: %s\n", __func__, pd->base.name);
				...
				u32 reset_flags)
{
	...
	pr_info("xzl: %s base %llx asb %llx is_2711 %d", __func__,
			(u64)power->base, (u64)power->asb, power->is_2711);

//	if (pm_reg == PM_GRAFX && reset_flags == PM_V3DRSTN)  { // xzl
//		pr_err("xzl: clk dev_id %s con_id %s", pd->clk->dev_id, pd->clk->con_id);
//		pr_err("xzl: skip to asb");
//		goto asb;
//	}
...
//	if (pm_reg == PM_GRAFX) // xzl
//		dev_err(power->dev, "okay to enable clock for %s\n",
//					pd->base.name);

	/* Wait 32 clocks for reset to propagate, 1 us will be enough */
	udelay(1);

	clk_disable_unprepare(pd->clk);		// disable and then unrepare... why?

	/* Deassert the resets. */
	PM_WRITE(pm_reg, PM_READ(pm_reg) | reset_flags);

	ret = clk_prepare_enable(pd->clk);	// xzl: enable again...
	if (ret) {
		dev_err(power->dev, "Failed to enable clock for %s\n",
			pd->base.name);
	...
	if (ret) {
		dev_err(power->dev, "Failed to enable ASB master for %s\n",
			pd->base.name);
//		if (pm_reg == PM_GRAFX) return ret; // xzl
		goto err_disable_clk;
	} else if (pm_reg == PM_GRAFX)
		dev_err(power->dev, "ok to enable ASB master for %s\n",
					pd->base.name);
...
#if 0 // for debugging exp
static int bcm2835_asb_power_on_xzl(struct bcm2835_power_domain *pd,
				u32 pm_reg,
				u32 asb_m_reg,
				u32 asb_s_reg,
				u32 reset_flags)
{
	...
	pr_err("xzl: %s base %llx asb %llx is_2711 %d", __func__,
			(u64)power->base, (u64)power->asb, power->is_2711);
...
//	goto asb;
	...
	/* Deassert the resets.
	 * xzl: letting the hw mod run? */
	{
		u32 val = PM_READ(pm_reg);
		printk(KERN_ERR "xzl: before. pmreg PM_GRAFX = %08x", val);
	}
...
asb: // xzl
	...
	if (ret) {
		...
		dev_err(power->dev, "xzl: ok to enable ASB master for %s\n",
					pd->base.name);
	...
	{
		u32 val = PM_READ(pm_reg);
		printk(KERN_ERR "xzl: after. pmreg PM_GRAFX = %08x", val);
	}
...
static int bcm2835_power_pd_power_on(struct generic_pm_domain *domain)
{
	...
	struct bcm2835_power *power = pd->power;
//	int ret; // xzl
//	void __iomem *addr = NULL; // xzl

	dev_info(power->dev, "xzl: %s: %s\n", __func__, pd->base.name);
...
#if 0 // xzl: try to read from the reg. debugging code
		{
			addr = ioremap(0xfec00000, 0x1000);
			if (addr) {
				pr_info("xzl: before power on. ioremap ok. addr = %llx val %08x",
						(u64)addr, readl(addr));
			} else
				pr_info("xzl: ioremap failed");
		}
		ret = bcm2835_asb_power_on_xzl(pd, PM_GRAFX,
					    ASB_V3D_M_CTRL, ASB_V3D_S_CTRL,
					    PM_V3DRSTN);
		if (addr) {
			addr = (char *)addr + 0x8;
			pr_info("xzl: after power on. ret %d ioremap ok. addr = %llx val %08x",
								ret, (u64)addr, readl(addr));
		}
#endif
...
static int bcm2835_power_probe(struct platform_device *pdev)
{
	...
	power->dev = dev;
	power->base = pm->base; // xzl: get the mapping from _pm..
	power->asb = pm->asb;
	 ...
	 * one for now.
	 *
	 * xzl: meaning ASB only serves rpivid?? no v3d on ASB now?
	 */
	if (pm->rpivid_asb) { // xzl: we have this. seems no real "asb" for bcm2711 now. not for v3d
		power->asb = pm->rpivid_asb;
		power->is_2711 = true;
	}

	id = ASB_READ(ASB_AXI_BRDG_ID);	// xzl: this uses rpivid_asb
