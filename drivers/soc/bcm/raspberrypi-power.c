...
struct rpi_power_domain {
	u32 domain;
	bool enabled;
	bool old_interface;		// xzl: why per domain? if old interface, use "set power state" tag
...
static int rpi_domain_off(struct generic_pm_domain *domain)
{
	...
	printk(KERN_INFO "xzl: %s: %s", __func__, domain->name);
					 ...
					 int xlate_index, const char *name)
{
	...
	printk(KERN_INFO "xzl: %s: %s: index %d", __func__, name, xlate_index);
}

// xzl: what's the diff between the following? the old interface has a domain
// idx?
...
static int rpi_power_probe(struct platform_device *pdev)
{
	...
	// xzl: unclear what power domains "new interface" means...
	...
	printk(KERN_INFO "xzl: %s ok. new interface %d\n",
			__func__, rpi_domains->has_new_interface ? 1 : 0);
